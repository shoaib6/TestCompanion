...
import com.example.testcompanion.RoomDatabase.SectionAccuracy
...
class ResultActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        loadSectionAccuracyChart(Constant.Category, Constant.Subject)
...
//    }
    ...
    private fun loadSectionAccuracyChart(category: String, subcategory: String) {
        val progressDao = appDatabase.progressDao()

        GlobalScope.launch(Dispatchers.Main) {
            val sectionAccuracyList = withContext(Dispatchers.IO) {
                progressDao.getSectionAccuracies(category, subcategory)
            }
            showAccuracyBarChart(sectionAccuracyList)
        }
    }

    private fun showAccuracyBarChart(data: List<SectionAccuracy>) {
        val barEntries = ArrayList<BarEntry>()
        val sectionLabels = ArrayList<String>()

        data.forEachIndexed { index, item ->
            barEntries.add(BarEntry(index.toFloat(), item.accuracy))
            sectionLabels.add(item.section)
        }

        val barDataSet = BarDataSet(barEntries, "Section Accuracy (%)")
        ...
        binding.barChart.apply {
            ...
            xAxis.valueFormatter = IndexAxisValueFormatter(sectionLabels)
            xAxis.position = XAxis.XAxisPosition.BOTTOM
            xAxis.setDrawGridLines(false)
            xAxis.granularity = 1f
            xAxis.labelCount = sectionLabels.size
    ...
    override fun onBackPressed() {
        super.onBackPressed()
        Constant.totalCorrectAnswers = 0
    }
